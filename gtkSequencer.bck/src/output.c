...
#define MAXNANOSECONDSTOSLEEPWHILEIDLE \
  ((maxSleepNanos > 1000000000) ? maxSleepNanos : 1000000000)
//#define MAXNANOSECONDSTOSLEEPWHILERUNNING 20000
					if (running && (nanosecondsToSleep > maxSleepNanos)) {
						nanosecondsToSleep = maxSleepNanos;
					}
				} else {
					nanosecondsToSleep = running ? maxSleepNanos : MAXNANOSECONDSTOSLEEPWHILEIDLE;
				}
				if (nanosecondsToSleep > 0) {
					...
					nanosecondsToSleep -= anticipation;
...
#define removeSyncEvent() doRemoveSyncEvent(TRUE, TRUE)
		...
		if (havePace) {
			...
			atMicrotick =
			  ...
			  + ((nanosecondsSinceLastTick + ant) / nanosecondsPerMicrotick);
		}
