...
static struct {
	...
	GtkWidget *lastLabel;
	//TODO: do we really need this mutex?
#if 0
...
{
	//TODO: can we do g_slist_free(signalling.labels); ?
...
static void enableButtons(void)
{
	...
	setEnabled(barsButton, TRUE);
	setEnabled(stepsPerBarButton, TRUE);
  ...
  uint32_t idx, GCallback cb, guint16 *shades, uint32_t shadesSize)
{
	...
	char *text = DISPLAYTEXT(step, TYPE(pattern));
  ...
  gboolean load, err_t *e)
{
	...
	if (!load) {
		count = g_slist_length(parent->children);
	}
  ...
  gboolean load, uint8_t channel, err_t *e)
{
	...
	terror(readWriteFd(&((*noteValue)->tone),
	  sizeof((*noteValue)->tone), fd, load, e))
	terror(readWriteFd(&((*noteValue)->sharp),
	  sizeof((*noteValue)->sharp), fd, load, e))
	terror(readWriteFd(&((*noteValue)->octave),
  ...
  pattern_t *parent, err_t *e)
{
	...
	if (!IS_DUMMY((*pattern))) {
		terror(readWriteFd(PTR_CHANNEL((*pattern)),
		  sizeof(CHANNEL((*pattern))), fd, load, e))
	}
	terror(readWriteFd(&NR_USERSTEPS_PER_BAR((*pattern)),
	  sizeof(NR_USERSTEPS_PER_BAR((*pattern))), fd, load, e))
	terror(readWriteFd(&NR_BARS((*pattern)),
	...
	if (!IS_DUMMY((*pattern))) {
		...
		if (!IS_NOTE((*pattern))) {
			terror(readWriteFd(PTR_PARAMETER((*pattern)),
...
static void cbLoad(GtkWidget *widget, gpointer data)
{
	...
	patterns.root->children =
	  g_slist_append(patterns.root->children, pattern);
	pattern = NULL;
...
static void cbDoAddPattern(GtkWidget *widget, gpointer data)
{
	...
	CHILDREN(createPattern.parent) =
	  g_slist_append(CHILDREN(createPattern.parent), pattern);
  ...
  *controllerValue)
{
	...
	char *name = (controllerValue == NULL) ? "" : controllerValue->name;
...
static GtkWidget *getSetupPatternDialog(void)
{
	...
	addWidget(GTK_BOX(contentArea), box);
	controllerSpinButton = getSpinButton(1, 127, 1);
...
static GtkWidget *getPatternListDialog()
{
	...
	for (GSList *cur = CHILDREN(patternList.parent); cur != NULL;
...
static GtkWidget *getValuesDialog(void)
{
	...
	for (cur = VALUES(current.pattern); cur != NULL; cur = g_slist_next(cur)) {
		...
		void *value = cur->data;
		box = getBoxWithLabel(GTK_ORIENTATION_HORIZONTAL,
		  VALUE_NAME(value, TYPE(current.pattern)));
		addWidget(GTK_BOX(contentArea), box);
