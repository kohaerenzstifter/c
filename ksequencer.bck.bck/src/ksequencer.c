  ...
  pattern_t *pattern, err_t *e)
{
	...
	for (uint32_t i = 0; i < NR_EVENTSTEPS(pattern); i++) {
		...
		unsoundNoteEvent(noteEventStep->onNoteEvent->on.offNoteEvent);
	}
  ...
  noteUserStep_t *noteUserStep, uint32_t userStepIdx)
{
	...
	void moveStepOnToNext(void) {
		noteEventStep_t *isNoteOnEventStep =
		  noteUserStep->onNoteEvent->noteEventStep;
	...
	void removeNote(void) {
		noteUserStep->onNoteEvent->on.offNoteEvent->noteEventStep->offNoteEvent
		  = NULL;
		free(noteUserStep->onNoteEvent->on.offNoteEvent);
  ...
  noteUserStep_t *noteUserStep, uint32_t userStepIdx)
{
	...
	if (onNoteEvent == NULL) {
		//TODO: valgrind says this is "definitely" lost
		onNoteEvent = calloc(1, sizeof(noteEvent_t));
...
gboolean anyChildStepSet(pattern_t *pattern, uint32_t idx)
{
	...
	for (GSList *cur = pattern->children; cur != NULL;
...
{
	g_slist_foreach(CHILDREN(pattern), cbFreePattern, NULL);
	g_slist_free(CHILDREN(pattern));
...
{
	free(controllerValue->snd_seq_event);
  ...
  gboolean live, err_t *e)
{
	...
	terror(setAlsaControllerEvent2(controllerValue->snd_seq_event, channel,
  ...
  lockContext_t *lockContext, err_t *e)
{
	...
	CHILDREN(parent) =
	  g_slist_delete_link(CHILDREN(parent), childLink);
...
static void noteOff(gpointer data, gpointer user_data)
{
	...
	snd_seq_event_output(sequencer.snd_seq,
	  noteEvent->noteValue->snd_seq_event);
